  ...
  case "$VVD_JTAG_MODE" in
    ...
    host)
      ...
      if [ "${VVD_NETWORK:-}" = "host" ]; then
        printf 'remote\tlocalhost\t%s' "$VVD_HW_SERVER_PORT"
      else
        printf 'remote\t%s\t%s' "$(engine_host_gateway_name)" "$VVD_HW_SERVER_PORT"
      fi ;;
    usb)
      printf 'usb\tlocalhost\t%s' "$VVD_HW_SERVER_PORT" ;;
    remote:*)
      local rest="${VVD_JTAG_MODE#remote:}" h p
      h="${rest%%:*}"
      p="${rest#*:}"
      [ "$p" = "$rest" ] && p="$VVD_HW_SERVER_PORT"
      [ -n "$h" ] || die "VVD_JTAG_MODE 'remote:' needs a host: remote:<host>[:<port>]"
      printf 'remote\t%s\t%s' "$h" "$p" ;;
    *)
      die "VVD_JTAG_MODE must be host, usb, remote:<host>[:<port>] or none (got: $VVD_JTAG_MODE)" ;;
  esac
}
...
jtag_usb_devices() {
  have lsusb || return 0
  local vid_re
  vid_re="$(IFS='|'; printf '%s' "${VVD_JTAG_VIDS[*]}")"
...
# jtag_args -- engine run arguments (one per line) plus the VVD_HW_SERVER_URL
...
jtag_args() {
  ...
  if [ "${VVD_JTAG_USB_ALL:-0}" -eq 1 ]; then
    # Survives replug (bus/device numbers change) at the cost of exposing every
    # USB device to the container.  Opt-in only.
    log_warn "passing the whole /dev/bus/usb tree into the container (VVD_JTAG_USB_ALL=1)"
  ...
  Fixes: plug the cable in, or run 'vvd jtag-rules --print' and install the udev
  rules so your user can open the device, or use the safer '--jtag host' mode."
  else
    for n in "${nodes[@]}"; do
      [ -r "$n" ] || log_warn "$n is not readable by $(id -un); see 'vvd jtag-rules --print'"
      printf -- '--device\n%s:%s:rwm\n' "$n" "$n"
    done
    log_debug "jtag: passing ${#nodes[@]} usb device node(s)"
  fi
  printf -- '--env\nVVD_HW_SERVER_URL=TCP:localhost:%s\n' "$port"
  printf -- '--env\nVVD_START_HW_SERVER=1\n'
}
