...
@test "verify-pinning passes on a clean tree" {
  run "$VVD_REPO_ROOT/scripts/verify-pinning.sh"
  [ "$status" -eq 0 ]
}

@test "verify-pinning catches an unpinned base image" {
  work="$TMP/repo"
  cp -r "$VVD_REPO_ROOT" "$work"
  rm -rf "$work/.git"
...
@test "every locked package carries a version and a sha256" {
  ...
  done <"$VVD_REPO_ROOT/docker/packages.lock"
}

@test "the apt snapshot is an immutable timestamp" {
  run grep -E '^APT_SNAPSHOT=[0-9]{8}T[0-9]{6}Z$' "$VVD_REPO_ROOT/docker/apt-snapshot.lock"
  [ "$status" -eq 0 ]
}
...
@test "images.lock entries are all digests" {
  ...
  done <"$VVD_REPO_ROOT/config/images.lock"
}
...
@test "the tool lock pins by sha256 or commit id" {
  while IFS='|' read -r kind name a b _; do
    ...
    : "$a"
  done <"$VVD_REPO_ROOT/scripts/tools.lock"
}

@test "no script fetches and executes in one step" {
  run grep -rnE '(curl|wget)[^|]*\|[[:space:]]*(ba)?sh' \
      "$VVD_REPO_ROOT/bin" "$VVD_REPO_ROOT/lib" "$VVD_REPO_ROOT/scripts" \
      "$VVD_REPO_ROOT/docker" "$VVD_REPO_ROOT/container"
  [ "$status" -ne 0 ]
}

@test "the Vivado installer digest is enforced by both the CLI and the image" {
  grep -q 'sha256sum' "$VVD_REPO_ROOT/lib/cmd/build.sh"
  grep -q 'sha256sum' "$VVD_REPO_ROOT/docker/install-vivado.sh"
}
...
@test "the build-time CA is never added to the runtime trust store" {
  # It lands under /opt/vvd/pin, never /etc/ssl/certs or the update-ca-certificates
  # source directory of the final stage.
  run grep -n 'ca-bootstrap' "$VVD_REPO_ROOT/docker/Dockerfile"
  [ "$status" -eq 0 ]
