...
@test "the suite's Tcl library is mounted read-only" {
  run vvd --dry-run synth
  assert_arg_pair "$output" --volume "$VVD_REPO_ROOT/tcl:/opt/vvd/tcl:ro"
}

@test "image mode mounts no Vivado tree" {
  run vvd --dry-run --image custom:tag synth
  VVD_VIVADO_MODE=image run vvd --dry-run synth
  [ "$status" -eq 0 ]
  refute_output_contains "$output" ":/opt/Xilinx:ro"
}

@test "mount mode fails clearly when the requested version is absent" {
  run vvd --vivado 2023.1 --dry-run synth
  [ "$status" -ne 0 ]
  [[ "$output" == *"Vivado 2023.1 not found"* ]]
}

@test "mount mode fails clearly when the Vivado root is absent" {
  VVD_XILINX_ROOT="$TMP/nope" run vvd --dry-run synth
  [ "$status" -ne 0 ]
  [[ "$output" == *"VVD_XILINX_ROOT does not exist"* ]]
}

@test "the invoking uid and gid are handed to the entrypoint" {
  run vvd --dry-run synth
  assert_arg_pair "$output" --env "VVD_UID=$(id -u)"
  assert_arg_pair "$output" --env "VVD_GID=$(id -g)"
}

@test "VVD_USER_MODE=root skips the identity mapping" {
  VVD_USER_MODE=root run vvd --dry-run synth
  refute_output_contains "$output" "VVD_UID="
}
...
@test "resource limits are passed through when configured" {
  VVD_MEMORY=16g VVD_CPUS=8 run vvd --dry-run synth
...
@test "extra mounts are honoured" {
  mkdir -p "$TMP/iprepo"
  VVD_EXTRA_MOUNTS="$TMP/iprepo:/opt/iprepo:ro" run vvd --dry-run synth
  assert_arg_pair "$output" --volume "$TMP/iprepo:/opt/iprepo:ro"
}

@test "an extra mount with a missing source fails" {
  VVD_EXTRA_MOUNTS="$TMP/absent:/opt/x:ro" run vvd --dry-run synth
  [ "$status" -ne 0 ]
...
@test "project settings reach the container as environment variables" {
  run vvd --dry-run synth
  assert_arg_pair "$output" --env VVD_TOP=top
  assert_arg_pair "$output" --env VVD_PART=xc7a35tcpg236-1
  assert_arg_pair "$output" --env "VVD_SOURCES=rtl/*.v"
}

@test "the synth stage is passed to flow.tcl" {
  run vvd --dry-run synth
  [[ "$output" == *"-source /opt/vvd/tcl/flow.tcl"* ]]
  [[ "$output" == *"-tclargs synth"* ]]
}

@test "flow runs every stage in one Vivado invocation" {
  run vvd --dry-run flow
  [[ "$output" == *"-tclargs all"* ]]
}
...
@test "build refuses an installer without a recorded digest" {
  ...
  run vvd build --installer "$TMP/fake-installer.tar"
  [ "$status" -ne 0 ]
  [[ "$output" == *"no SHA256 recorded"* ]]
}

@test "pull refuses an unpinned reference" {
  run vvd pull docker.io/example/vivado:latest
  [ "$status" -ne 0 ]
